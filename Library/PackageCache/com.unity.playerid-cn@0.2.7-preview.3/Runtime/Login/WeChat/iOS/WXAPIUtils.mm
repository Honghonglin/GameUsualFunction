...
+ (void)registerApp:(const char*)link {
    NSArray * urlTypes = [[[NSBundle mainBundle] infoDictionary] valueForKey:@"CFBundleURLTypes"];
    NSString * appID;
    NSString * universal_link=[NSString stringWithUTF8String:link];
...
+ (void)loginWeChat {
    ...
    [WXApi sendReq:request completion:nullptr];
}
...
extern "C" {
    ...
    void registerWxAPI(const char* link) {
        [WXAPIUtils registerApp:link];
    }
