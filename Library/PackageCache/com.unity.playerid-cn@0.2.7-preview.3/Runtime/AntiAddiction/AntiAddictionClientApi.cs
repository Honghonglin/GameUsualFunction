        ...
        IEnumerator Post(string cmd, string data, int type)
        {
            ...
            if (request.isNetworkError)
            {
                if (OnJudgeActionRigistered(OnMessage,"OnMessage",3))
                    OnMessage("AntiAddictionNetworkError", request.error+request.downloadHandler.text, "");
            } 
            else if (request.isHttpError)
            {
                if (OnJudgeActionRigistered(OnMessage,"OnMessage",3))
                    OnMessage("AntiAddictionServerError", request.error+request.downloadHandler.text, "");
            }
            else
            {
